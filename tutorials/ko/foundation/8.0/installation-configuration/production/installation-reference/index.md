...
다음과 같은 단일 데이터베이스를 선언해야 합니다. `<database kind="ApplicationCenter">`. `<database>` 요소는 `<configuredatabase>` Ant 태스크와 유사하게 지정합니다. 다만, `<database>` 요소에는 `<dba>` 및 `<client>` 요소가 없습니다. `<property>` 요소에 추가하십시오.
