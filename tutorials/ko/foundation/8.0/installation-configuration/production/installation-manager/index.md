...
사용자 모드에서는 특정 권한이 없는 임의의 사용자가 Installation Manager를 실행할 수 있습니다. 단, Installation Manager의 저장소 파일은 해당 사용자의 홈 디렉토리에 저장됩니다. 해당 사용자만 제품의 설치를 업그레이드할 수 있습니다.
Installation Manager를 루트로 실행하지 않는 경우, 나중에 제품 설치를 업그레이드하거나 임시 수정사항을 적용할 때 사용할 수 있는 사용자 계정이 있는지 확인하십시오.
...
<table style="word-break:break-all">
    ...
    <tr>
      ...
      <td>${user.database.selection2} == oracle(${user.database.oracle.appcenter.jdbc.url}을 지정하지 않은 경우에 한함)</td>
      <td>Application Center용 Oracle 데이터베이스의 이름입니다.</td>
      <td>비어 있지 않음, 유효한 Oracle 데이터베이스 이름.
</td>
    </tr>
    <tr>
      <td>user.database.oracle.appcenter.isservicename.jdbc.url</td>
