...
Ant 파일이 저장됩니다. Server Configuration Tool은 구성 설치 및 업데이트를 위한 Ant 파일의 작성을 지원합니다. 이 Ant 파일은 **파일 → Ant 파일로 구성 내보내기...**를 사용하여 내보낼 수 있습니다. 이 Ant 파일에 대한 자세한 정보는 [명령행 모드에서](../command-line) {{ site.data.keys.mf_server }} 설치에서 Ant 태스크를 사용하여 Liberty에 {{ site.data.keys.mf_server }} 배치를 참조하십시오.
