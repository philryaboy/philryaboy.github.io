...
## 개요
{: #overview }
아래의 지시사항에 따라 IBM Cloud에서 Liberty for Java 런타임에 {{ site.data.keys.mf_server }} 인스턴스를 구성하십시오. <!--({{ site.data.keys.mf_analytics }} instances can be run on IBM containers only.)--> 이를 달성하기 위해 다음 단계를 수행합니다.

* 필요한 도구(Cloud Foundry CLI)를 사용하여 호스트 컴퓨터 설정
* IBM Cloud 계정 설정
* {{ site.data.keys.mf_server }}를 빌드하여 IBM Cloud에 Cloud Foundry 애플리케이션으로 푸시
...
* [IBM Cloud에 계정 등록](#register-an-account-at-ibmcloud)
            ...
            <div class="panel-body">
              ...
              <ol>
                  <li><b>initenv.sh – IBM Cloud에 로그인</b><br />
                      <b>initenv.sh</b> 스크립트를 실행하여 IBM Cloud에 로그인하십시오. dashDB 서비스가 바인드되는 조직과 영역에 대해 다음을 실행하십시오.
{% highlight bash %}
...
{% endhighlight %}
                  ...
                  <b>prepareappcenterdbs.sh</b> 스크립트는 dashDB 데이터베이스 서비스 또는 액세스 가능한 DB2 데이터베이스 서버를 사용해서 {{ site.data.keys.mf_app_center }}를 구성하는 데 사용됩니다. DB2 옵션은 특히 DB2 서버가 설치된 동일한 데이터 센터에서 IBM Cloud Local을 실행 중인 경우 사용 가능합니다. dashDB 서비스를 사용하는 경우 1단계에서 로그인한 조직과 영역에서 dashDB 서비스의 서비스 인스턴스를 사용할 수 있어야 합니다. 다음을 실행하십시오.
{% highlight bash %}
...
{% endhighlight %}
                  ...
                  <li><b>initenv.sh(선택사항) – IBM Cloud에 로그인</b><br />
                      이 단계는 dashDB 서비스 인스턴스를 사용할 수 있는 조직과 영역 이외의 조직과 영역에서 서버를 작성해야 하는 경우에만 필수입니다. 값이 예인 경우에는 컨테이너가 작성되고 시작되어야 하는 새 조직과 영역으로 initenv.properties를 업데이트하고 <b>initenv.sh</b> 스크립트를 다시 실행하십시오.
{% highlight bash %}
./initenv.sh args/initenv.properties
{% endhighlight %}
                  </li>
...
{% endhighlight %}
                  ...
                  <b>prepareserverdbs.sh</b> 스크립트는 dashDB 데이터베이스 서비스 또는 액세스 가능한 DB2 데이터베이스 서버를 사용해서 {{ site.data.keys.mf_server }}를 구성하는 데 사용됩니다. DB2 옵션은 특히 DB2 서버가 설치된 동일한 데이터 센터에서 IBM Cloud Local을 실행 중인 경우 사용 가능합니다. dashDB 서비스를 사용하는 경우 1단계에서 로그인한 조직과 영역에서 dashDB 서비스의 서비스 인스턴스를 사용할 수 있어야 합니다. 다음을 실행하십시오.
{% highlight bash %}
...
#### 변경사항 적용
{: #applying-changes }
서버를 배치한 후 서버 레이아웃에 변경사항을 적용해야 할 수 있습니다. 예를 들면, **/usr/config/mfpfproperties.xml**에서 Analytics URL을 업데이트할 수 있습니다. 변경한 후 동일한 인수 세트를 사용해서 다음 스크립트를 다시 실행하십시오.
